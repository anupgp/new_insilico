...
namespace newinsilico
{
  ...
  auto get_neuron_index(unsigned _id, std::string _variable) -> unsigned {
    ...
    if(index_map.find(key) == index_map.end()) {
      std::cerr << "[newinsilico::neuron_index] Failed to find "<<_variable
  ...
  auto get_synapse_index(unsigned _id, std::string _variable) -> unsigned {
    ...
    if(index_map.find(key) == index_map.end()) {
      std::cerr << "[newinsilico::synapse_index] Failed to find "<<_variable
  ...
  auto get_neuron_index(unsigned _id, std::string _variable, bool& error)
    -> unsigned {
    char key[128];
    sprintf(key, "n%d%s", _id, _variable.c_str());
    if(index_map.find(key) == index_map.end()) {
  ...
  auto get_synapse_index(unsigned _id, std::string _variable, bool& error)
    -> unsigned {
    char key[128];
    sprintf(key, "s%d%s", _id, _variable.c_str());
    if(index_map.find(key) == index_map.end()) {
  ...
  auto get_neuron_indices(std::string _variable) -> std::vector<unsigned> {
    ...
    for(unsigned index = 0; index < total_neurons; ++index) {
      idx = get_neuron_index(index, _variable, error);
  ...
  auto get_synapse_indices(std::string _variable) -> std::vector<unsigned> {
    ...
    for(unsigned index = 0; index < total_synapses; ++index) {
      idx = get_synapse_index(index, _variable, error);
      if(!error) { indices.push_back(idx); }
      error = false;
    }
    return indices;
  ...
  auto neuron_id_from_index(unsigned _index) -> unsigned {
    if(prepopulated_neuron_ids.empty() ||
      ...
      std::cerr << "[newinsilico::neuron_id_from_index] "
  ...
  auto synapse_id_from_index(unsigned _index) -> unsigned {
    if(prepopulated_synapse_ids.empty() ||
      ...
      std::cerr << "[newinsilico::synapse_id_from_index] "
		<< "Failed to find index "<<_index <<'\n';
      exit(0);
    }
  ...
  auto get_pre_neuron_indices(unsigned _id, std::string _variable)
	...
	idx = get_synapse_index(pre_synaptic_lists[_id][index], _variable, error);
	if(!error) {
