  ...
  void read_neuron_synapse(const std::string neuron_file, const std::string synapse_file) {
	...
	if(part.length() > 0) {
	  ...
	  while(getline(l, part, worddelim)) {
		...
		prepopulated_neuron_ids.push_back(ntrack);
		var_list_ids.push_back(first_item);
		var_vals.push_back(second_item);
		var_noise.push_back(0.0);
		index_map[key] = ncount;
		++ncount;
		--dxdt_count;
	      }
	      // universal value map
	      value_map[key] = second_item;
	    }
	  }
	  ntrack+=1;
	...
	if(part.length() > 0) {
	  ...
	  while(getline(l, part, worddelim)) {
		...
		pre_neuron.push_back((int)(ceil(second_item)));
	      }
	      else if(first_item.compare("post") == 0) {
		post_neuron.push_back((int)(ceil(second_item)));
	      }
	      else if(dxdt_count > 0) {
		prepopulated_synapse_ids.push_back(strack);
		var_list_ids.push_back(first_item);
		var_vals.push_back(second_item);
		var_noise.push_back(0.0);
		index_map[key] = ncount;
